...
def test_adds_zip_id_to_data_frame():
    ...
    assert result_df.shape == (resulting_rows, 3)
    assert result_df.columns[0] == 'id'
    assert result_df.columns[1] == 'time'
    assert result_df.columns[2] == 'bg'
    assert 19 in result_df.index  # this is testing reindexing after concatenation
    assert result_df1[['time', 'bg']].equals(df1)
    assert result_df2[['time', 'bg']].reset_index(drop=True).equals(df2)
...
def test_skips_records_with_no_bg_data():
    ...
    assert result_df.shape == (df1.shape[0] + df2.shape[0], 3)
...
def test_drops_rows_with_nan_value():
    ...
    # assert resulting dataframe contains both df 1 and df2
    assert result_df.shape == (df1.shape[0] - nans, 3)
...
def test_only_keeps_columns_specified():
    ...
    assert set(result_df.columns) == {'id', 'bg'}
