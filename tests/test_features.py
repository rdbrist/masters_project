...
def test_load_preprocessed_data_missing_file(tmp_path):
    ...
    with pytest.raises(FileNotFoundError, match=re.escape(f"File {missing_file} not found.")):
...
def test_load_preprocessed_data_invalid_index_structure(tmp_path):
    ...
    data = {"id": [1, 2], "datetime": ["2023-01-01", "2023-01-02"]}
    df = pd.DataFrame(data)
...
def test_add_time_based_features_creates_correct_columns_with_valid_data(tmp_path, simple_df):
