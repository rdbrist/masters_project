...
from src.data_processing.preprocess import dedup_device_status_dataframes, group_into_consecutive_intervals, \
...
def test_removes_duplicated_device_status_rows_from_each_df_in_read_record():
    ...
    assert df1.shape[0] == (10)  # ensure the builder doesn't dedup
    assert df2.shape[0] == (10)

    results = dedup_device_status_dataframes([test_record1, test_record2])

    # assert resulting dataframe has been deduped
    number_of_cols = len(Configuration().device_status_col_type)
    assert len(results) == 2
    assert results[0].df.shape == (10 - dub_1, number_of_cols)
    assert results[1].df.shape == (10 - dub_2, number_of_cols)
...
def test_can_deal_with_missing_columns():
    ...
    result = dedup_device_status_dataframes([test_record])
    number_of_cols = len(Configuration().device_status_col_type)
    assert result[0].df.shape == (10 - dub_1, number_of_cols - len(drop_cols))


def test_can_deal_with_none_df():
    record = ReadRecord()
    result = dedup_device_status_dataframes([record])
...
def test_groups_df_into_consecutive_sampling_time():
    ...
    assert len(result['group'].unique()) == 3  # three groups of 5 min intervals
    group1 = result.loc[result['group'] == 0]
    group2 = result.loc[result['group'] == 1]
    group3 = result.loc[result['group'] == 2]
    assert list(group1['bg']) == values1
    assert list(group1['time']) == times1
    assert list(group2['bg']) == values2
    assert list(group2['time']) == times2
    assert list(group3['bg']) == values3
    assert list(group3['time']) == times3
...
def test_returns_list_of_continuous_series_of_length_x_and_sampled_at_interval():
    ...
    # build time series
    date1 = datetime(year=2018, month=12, day=25, hour=0, minute=0, tzinfo=timezone.utc)
    date2 = datetime(year=2020, month=3, day=14, hour=6, minute=11, tzinfo=timezone.utc)
    date2_more = date2 + timedelta(minutes=10)
    date3 = datetime(year=2021, month=8, day=14, hour=0, minute=0, tzinfo=timezone.utc)
    ...
    assert len(result) == 2  # There wasn't 2 continuous sampled sub series
    assert list(sub_df1[time_col]) == times1
    assert list(sub_df2[time_col]) == times3


def test_removes_days_that_have_only_partial_data():
    time_col = 'time'
    max_gap_in_min = 15  # sampling interval
    # only keep continuous 15 sampled series for 2 days
    min_daily_length = number_of_interval_in_days(1, max_gap_in_min)
    min_length = number_of_interval_in_days(2, max_gap_in_min)
    ...
    assert len(result) == 1  # there's  no gap
    assert result[0].shape[0] == min_length  # removed the dates that didn't have fully sampled data
